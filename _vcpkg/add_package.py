...
Dependency = TypedDict("Dependency", {"name": str, "default-features": bool, "features": List[str]})
...
def validate_object(model: Any, instance: Any) -> bool:
    ...
    if ctor in (list, set):
        return isinstance(instance, ctor) and all(validate_object(params[0], value) for value in instance)

    if ctor is dict:
        return isinstance(instance, ctor) and all(
            validate_object(params[0], key) and validate_object(params[1], value) for key, value in instance.items()
        )
    ...
    for prop_name, prop_type in model.__annotations__.items():
        ...
        if extracted_value is sentinel or not validate_object(prop_type, extracted_value):
            return False
...
def add_package(args: argparse.Namespace) -> None:
    vcpkg_json_path = args.outdir / "vcpkg.json"
    ...
    if vcpkg_json_path.exists():
        ...
        if not validate_object(VCPkgJson, vcpkg_json):
            raise AssertionError(f"Validation failed: {vcpkg_json_path.name} didn't match type annotations")

    match = re.match(r"^([a-z0-9-]+)(?:\[(.+)])?$", args.name)

    if not match:
        raise AssertionError(f"Validation failed: {args.name!r} is not a valid package name")

    package_name, package_features = match.groups(default="")
    package_feature_list: List[str] = list(filter(None, map(str.strip, package_features.split(","))))
    ...
    for index, dependency in enumerate(vcpkg_json["dependencies"]):  # type: int, Dependency
    ...
    if args.version:
        for index, override in enumerate(vcpkg_json["overrides"]):  # type: int, Override
...
if __name__ == "__main__":
    ...
    print(f"-- vcpkg: Adding package {parsed_args.name}@{parsed_args.version or 'latest'}")
